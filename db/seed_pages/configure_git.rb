...
previous_steps = Step.joins(:step_content).where("step_contents.title = 'Install Rails' OR step_contents.title = 'Update Rails' OR step_contents.title = 'Install Git' ")

previous_steps.each do |step|
  Step.create(step_content_id: step_content.id, previous_step_id: step.id)
end

previous_steps = Step.joins(:step_content).where("step_contents.title = 'Verify Rails Version'")

previous_steps.each do |step|
  Step.create(step_content_id: step_content.id, previous_step_id: step.id, button_text: '4.0')
end